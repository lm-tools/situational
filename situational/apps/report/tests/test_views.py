...
class TestReportView(TestCase):
    ...
    def test_get_with_populated_report(self):
        with patch('report.models.Report.is_populated', new_callable=PropertyMock) as mock_is_populated:
            mock_is_populated.return_value = True

            response = self.client.get(reverse('report_view', kwargs={'postcode': 'SW1H0DJ'}))
